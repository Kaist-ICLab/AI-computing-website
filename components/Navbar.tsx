...
const Navbar: React.FC<NavbarProps> = ({ lang, setLang, t, isScrolled, setPage, currentPage }) => {
  ...
  const navItems: NavItem[] = [
    { 
      ...
      label: t.intro,
      sub: [
        { label: lang === 'ko' ? '학과장 인사말' : 'Welcome Message', action: () => setPage('welcome-message') },
        { label: lang === 'ko' ? '학과 소개' : 'AI Computing Dept Introduction', action: () => setPage('dept-intro') }
      ]
    },
    { 
      id: 'people-section', 
      label: t.people, 
      sub: [
        { label: lang === 'ko' ? '교수진' : 'Faculty', action: () => setPage('people') }
      ] 
    },
    ...
    { 
      ...
      label: t.education,
      sub: [
        { label: lang === 'ko' ? '교과목 안내' : 'Course Guide', action: () => setPage('education-courses') },
        { label: lang === 'ko' ? '이수요건' : 'Requirements', action: () => setPage('education-reqs') }
      ]
    },
  ...
  const isEducationActive = currentPage === 'education-courses' || currentPage === 'education-reqs';
  const isIntroActive = currentPage === 'welcome-message' || currentPage === 'dept-intro';

  return (
    <nav 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        showDarkNavbar ? 'bg-transparent' : 'bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100'
      }`}
      ...
      <div ref={navContainerRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-4 group cursor-pointer" onClick={() => setPage('home')}>
              ...
              <div className="flex flex-col items-start leading-none">
                <span className={`font-black text-lg md:text-xl tracking-tighter italic transition-colors ${logoColor}`}>
                  COLLEGE OF AI
                </span>
                <span className={`font-extrabold text-[8px] tracking-[0.2em] uppercase transition-colors ${
          ...
          <div className="hidden md:flex items-center space-x-12 h-full">
            {navItems.map(item => {
              const isActive = (item.id === 'intro-section' && isIntroActive) ||
              ...
              return (
                <div 
                  ...
                  onMouseEnter={() => item.sub.length > 0 ? setActiveMenu(item.id) : setActiveMenu(null)}
                >
                  <button 
                    onClick={() => {
                    ...
                    className={`flex items-center gap-1 font-black text-xs transition-all uppercase tracking-widest relative py-2 ${
                      !showDarkNavbar 
                      ? (isActive ? 'text-[#004191]' : 'text-slate-700 hover:text-[#004191]') 
                      : 'text-white/80 hover:text-white'
                    }`}
                  >
                    {item.label}
                    <span className={`absolute bottom-0 left-0 h-0.5 transition-all group-hover:w-full ${
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col space-y-1">
            {navItems.find(i => i.id === activeMenu)?.sub.map((sub, idx) => (
              <button
                key={idx}
